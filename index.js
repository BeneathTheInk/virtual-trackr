...
Virtual tracker is designed to
- generate virtual dom nodes
- within an autorun context
- as a virtual dom widget.
...
var diff = require('./lib/multidiff.js');
var patch = require('./lib/multipatch.js');
...
// returns an array with the parent and nextSibling nodes of this widget
VTrackr.prototype.position = function() {
	var before, parent;
	var lastNode = _.last(this.nodes);

	if (lastNode) {
		before = lastNode.nextSibling;
		parent = lastNode.parentNode;
	} else {
		before = this.placeholder;
	}

	if (before && !parent) parent = before.parentNode;

	return before && parent ? new utils.Position(parent, before) : null;
};

VTrackr.prototype.draw = function() {
	// must have changed
	...
	if (!pos) return this.clearNodes();

	// create new nodes if none is set
	if (!this.nodes || !this._tree) {
		this.nodes = this._latestTree.map(function(t) {
	...
	// otherwise patch and apply new tree
		...
		this.nodes = multivdom.patch(
			pos, this.nodes.slice(0),
			multivdom.diff(this._tree, this._latestTree)
		);
	}

	// apply all nodes onto the dom
	utils.insertNodes(this.nodes, pos);
...
VTrackr.prototype.updateTree = function(newtree) {
	this._latestTree = newtree;
	this.draw();
	return this;
};

VTrackr.prototype.clear = function() {
	if (this.nodes) this.nodes.forEach(function(n) {
...
// called by vdom when being freshly generated
VTrackr.prototype.init = function() {

};

// called by vdom when new, but replacing something existing
VTrackr.prototype.update = function() {

};

// called when removed from the dom or needs to be otherwise killed
VTrackr.prototype.destroy = function() {
